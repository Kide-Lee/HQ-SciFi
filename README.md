...
- **阅读**：推荐（精选 / AI 模型 / 首页置顶与内容流）、连载（推荐合集 / 推荐连载）、练笔活动（近期活动 / 活动树展开文章）、作品库五大栏目；正文安全净化渲染；列表排序支持评分 / 点赞 / 评论 / 阅读 / 字数 / 时间 / 回复，全部可倒序（↑/↓），并列排名同名次；**未登录也可浏览与匿名阅读公开文章**（仅当服务端确认文章不可读时才提示登录，网络故障不误标）
- **用户系统**：点击头像进入用户页——展示昵称 / 等级 / UID / 粉丝 / 文章 / 评论（本人页含关注）与主页背景；主页四栏（收藏 / 文章 / 评审 / 评论）与子页（动态 / 收藏 / 粉丝 / 文章 / 评审 / 评论）；关注动态聚合、签到领能量币、统计直达子页；**已登录用户启动直接进入本人用户页**；投稿期 / 评稿期活动文章与评审全站匿名且不暴露评分
...
- **安全**：登录凭据经系统级加密（safeStorage）仅存主进程、渲染层沙箱隔离、远端内容白名单净化防 XSS；**首次启动须阅读并同意《黄芪饮片用户协议》**（滚动到底才能勾选）；**每次启动自动校验会话有效性**，token 失效自动丢弃并退回未登录（网络异常不误登出）；任意界面 Ctrl+F12 打开开发者工具
...
> **本地测试模式**（对接自建 RuleApi / 本地接口联调）：`npm run dev:test`（或 `electron out/main/index.js --test` / 环境变量 `HQSF_TEST=1`；跨平台由 `cross-env` 注入，Windows cmd/PowerShell 亦可用）。测试模式与正式使用完全隔离——独立 userData（`~/.config/hqsf-test`，数据库/session/设置不混入正式库）、API 配置优先读本地持有的 `api.config.test.json`（baseUrl 指向本地服务，不入库）、默认存档根为 `~/文档/荒启科幻/草稿-test`。正式模式（`npm run dev`）不受任何影响，两者可同时运行。
...
1. 启动客户端：**首次启动先阅读并同意《黄芪饮片用户协议》**方可进入；未登录也可直接本地写作、浏览栏目、匿名阅读公开文章；**已登录用户启动后直接进入本人用户页**
2. 登录（左栏用户卡「点击登录」，账号密码）后启用同步 / 发布 / 评论 / 评审；**每次启动自动校验会话**，token 失效自动退回未登录；登录后可在左栏用户卡**签到**领能量币
3. **用户页**：点击左栏头像或任意用户头像进入；主页与子页可查看收藏 / 文章 / 评审 / 评论、关注 / 粉丝与动态；点击头部统计可直达对应子页
4. **写作**：左侧「写作」→ 新建草稿 → 编辑（自动保存）→ 「同步到草稿」或「发布」（发布表单填写类型 / 标签后提交，发布后进入官方审核流程，审核通过即公开）
5. **阅读**：通过左侧栏目（推荐 / 连载 / 活动 / 作品库）浏览文章，点击卡片进入阅读
6. **评审**：打开他人文章 → 「✎ 评审这篇文章」→ 填写五维评分与评语 → 提交
