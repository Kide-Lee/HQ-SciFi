...
1. 启动客户端：**首次启动先阅读并同意《黄芪饮片用户协议》**方可进入；未登录也可直接本地写作、浏览栏目、匿名阅读公开文章
2. 登录（左栏用户卡「点击登录」，账号密码）后启用同步 / 发布 / 评论 / 评审；**每次启动自动校验会话**，token 失效自动退回未登录
3. **写作**：左侧「写作」→ 新建草稿 → 编辑（自动保存）→ 「同步到草稿」或「发布」（发布表单填写类型 / 标签后提交，发布后进入官方审核流程，审核通过即公开）
4. **阅读**：通过左侧栏目（推荐 / 连载 / 活动 / 作品库）浏览文章，点击卡片进入阅读
5. **评审**：打开他人文章 → 「✎ 评审这篇文章」→ 填写五维评分与评语 → 提交
