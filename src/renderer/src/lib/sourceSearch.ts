...
import { escapeRegExp, wrapIndex } from './searchText'
/**
...
export const sourceSearchParams: SourceSearchParams = { query: '', regex: false, active: 0 }

/** 外部触发装饰重算的事务 effect */
const refreshEffect = StateEffect.define<void>()

/** 构造 RegExpCursor 的模式（正则非法返回 null）；忽略大小写与 SearchPanel 一致 */
function patternOf(params: SourceSearchParams): { pattern: string; ignoreCase: boolean } | null {
...
/** 文档中全部匹配区间（文档顺序，与 SearchPanel 同序） */
